...
Сценарий: 0006 проверка проведения док.Продажи
		...
		* проверка проведения док.Продажи
			...
			И я жду, что в табличном документе "Результат" ячейка "R13C3" станет равна "300,00" в течение 20 секунд
			...
			Тогда открылось окно 'Продажа * от *'
			И я нажимаю на кнопку с именем 'ОстаткиПоСкладу'
			И я жду, что в табличном документе "Результат" ячейка "R13C3" станет равна "290,00" в течение 20 секунд
			И я закрываю текущее окно
		...
		* проверка печатной формы Расходная товарная накладная			
			...
			И в табличном документе 'SpreadsheetDocument' я перехожу к ячейке "R12C4"
